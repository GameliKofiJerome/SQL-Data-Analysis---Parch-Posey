...
INNER JOIN
parch_and_posey.dbo.accounts a
...
total, so we will have to divide by (total + 0.01) to assure not dividing by zero.
*/
...
the accounts where the sales_rep has a first name starting with S and in the Midwest region. 
...
--OR / ALTERNATIVE SOLUTION WITHOUT THE WHERE CLAUSE
...
/*
Provide a table that provides the region for each sales_rep, along with their associated accounts. This time only focus on 
the accounts where the sales_rep has a last name starting with K and in the Midwest region. 
Your final table should include 3 columns: the region name, the sales_rep name, and the account name. 
Sort the accounts alphabetically (A - Z) according to account name.
*/
...
NB: A few accounts have 0 for total, so we will have to divide by (total + 0.01) to assure not dividing by zero.
*/
...
/*
What are the different channels used by the account with id 1001? The final table should have only 2 columns: account name and
...
-- Find all orders that occured in 2015. Your final table should have 4 columns: occurred_at, account_name, order total 
